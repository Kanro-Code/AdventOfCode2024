...
impl <T> Iterator for GridIterator<'_, T> where T: Clone {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        if let Some(current) = &self.current {
            ...
            self.next = self.calculate_next_point();
            value
        } else {
            None
        }
...
mod tests {
    ...
    pub fn test_new_grid_iterator() {
        ...
        assert_eq!(iter.current_point(), &Some(Point { x: 0, y: 0 }));
        assert_eq!(iter.next_point(), &Some(Point { x: 1, y: 0 }));

        assert_eq!(iter.next(), Some(1));
        assert_eq!(iter.current_point(), &Some(Point { x: 1, y: 0 }));
        assert_eq!(iter.next_point(), &Some(Point { x: 2, y: 0 }));
        ...
        let iter = grid.iter();
        assert_eq!(iter.current_point(), &Some(Point { x: 0, y: 0 }));
        assert_eq!(iter.next_point(), &None);
